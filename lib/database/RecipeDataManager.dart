...
class RecipeDatabaseManager {
  ...
  static Future<int> upsertRecipe(Recipe recipe) async {
    ...
    if (recipe.steps != null && recipe.steps.length > 0) {
      recipe.steps.forEach((element) {
        upsertFutures.add(
            db.insert(RecipeDatabase.stepsTable, element.toMap(recipeId)));
      });
    }
  ...
  static Future<void> deleteRecipe(Recipe recipe) async {
    ...
    await db.delete(RecipeDatabase.ingredientsTable, where: "recipe_id = ?", whereArgs: [recipe.id]);
    await db.delete(RecipeDatabase.stepsTable, where: "recipe_id = ?", whereArgs: [recipe.id]);

    return db.delete(RecipeDatabase.recipeTable, where: "id = ?", whereArgs: [recipe.id]);
  }
  ...
  static Future<List<Recipe>> allRecipes() async {
    ...
    return List.generate(maps.length, (i) {
      return Recipe(
        id: maps[i]['id'],
        name: maps[i]['name'],
        meatContent: Utils.cast<String>(maps[i]['meat_content']).toMeatContent(),
      );
    });
  }
}